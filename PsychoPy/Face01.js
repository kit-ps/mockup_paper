...
let expName = 'Face01';  // from the Builder filename that created this script
let expInfo = {
    '': '',
};
...
function StimuliRoutineBegin(snapshot) {
  return async function () {
    ...
    if (((r < 0.333) && (flage === 0))) {
        ...
        photo.concat(getRandomInt(1, 70).toString(),'.jpg');
        value = 2;
        flage = 1;
    } else {
            photo = "/Faces/Familiar/";
            photo.concat(getRandomInt(1, 123).toString(),'.jpg');
            value = 1;
            flage = 0;
        }
    delay = (1 + Math.random());
