...
flowScheduler.add(sentence_series_part4RoutineBegin());
flowScheduler.add(sentence_series_part4RoutineEachFrame());
flowScheduler.add(sentence_series_part4RoutineEnd());
...
async function experimentInit() {
  ...
  welcome = new visual.TextStim({
    win: psychoJS.window,
    name: 'welcome',
    text: 'Please read the following sentences while we register your brainwaves \n\n\n Press "Space" to start. ',   font: 'Arial',
    units: undefined, 
...
function sentence_series_part1RoutineEachFrame() {
  return async function () {
    ...
    // *text_113* updates
    if (t >= 0.5 && text_113.status === PsychoJS.Status.NOT_STARTED) {
      // keep track of start time/frame for later
    ...
    // *ext_114* updates
    if (t >= 1 && ext_114.status === PsychoJS.Status.NOT_STARTED) {
      // keep track of start time/frame for later
...
function final_step() {
  window.location.href = "/success?t=sentence_v";
 }  